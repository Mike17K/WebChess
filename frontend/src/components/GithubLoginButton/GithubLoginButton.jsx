...
export default function GithubLoginButton({setJwt,userData,setUserData}) {
    ...
    useEffect(()=>{
        ...
        if(session !== null){
            if(session.provider === "Github" && queryParams.github_access_token === undefined) {
                setUserData(session);
    ...
    useEffect(()=>{
        ...
        console.log("Fetching user data Github",userData)
        fetch(`${GITHUB_ENDPOINT}/user`, {
        method: 'GET',
        headers: {
            'Authorization': `token ${queryParams.github_access_token}`
        }
      }).then(response=> response.json()).then((data) => {
        // do something
        setUserData(data)
        setJwt(data.access_token)
        setAccessToken(queryParams.github_access_token)
        localStorage.removeItem('session');
        localStorage.setItem('session', JSON.stringify({...data,access_token:queryParams.github_access_token,provider:"Github"}));
    ...
    // for discord logout 
    function logout(e){       
        fetch(`http://localhost:5050/api/users/auth/github/logout`, {
            method: 'GET',
            headers: {
                'token': accessToken
            }
            ...
            console.log('access tocken revoked')
            setJwt("");
            setUserData({});
            localStorage.removeItem('session');
        }).catch(err => {
            console.log(err);
            console.log('access tocken not revoked')
        }); 
  ...
  return (
      <>
        {
            (userData.provider !== "Github" || userData.access_token === undefined) && (
        <button id="info" 
        ...
        <img className="w-[22px] h-[22px] rounded-full ml-[10px]" src={`${userData.avatar_url}`} alt="Discord Avatar" />
        ...
        <div className='block text-sm my-auto px-2 leading-[15px]'>
            ...
            <div className='block text-white text-[12px] text-start'>{userData.login}</div>
